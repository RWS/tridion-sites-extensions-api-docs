<!-- Do not edit this file. It is automatically generated by API Documenter. -->

[Home](./index.md) &gt; [@tridion-sites/extensions](./extensions.md) &gt; [contentExplorerTreeActionId](./extensions.contentexplorertreeactionid.md)

## contentExplorerTreeActionId variable

**Signature:**

```typescript
contentExplorerTreeActionId: {
    export: "export";
    refresh: "refresh";
    openEditor: "openEditor";
    openEditorInNewTab: "openEditorInNewTab";
    addSelectedItemsToBundle: "addSelectedItemsToBundle";
    addToBundle: "addToBundle";
    removeFromBundle: "removeFromBundle";
    autoClassify: "autoClassify";
    copy: "copy";
    cut: "cut";
    paste: "paste";
    newBundle: "newBundle";
    newCategory: "newCategory";
    newComponent: "newComponent";
    newFolder: "newFolder";
    newKeyword: "newKeyword";
    newMultimediaComponent: "newMultimediaComponent";
    newPage: "newPage";
    newStructureGroup: "newStructureGroup";
    uploadMultimediaComponent: "uploadMultimediaComponent";
    delete: "delete";
    finishEditing: "finishEditing";
    revert: "revert";
    localize: "localize";
    unlocalize: "unlocalize";
    publish: "publish";
    unpublish: "unpublish";
    openPublishingQueue: "openPublishingQueue";
    openTranslationQueue: "openTranslationQueue";
    translate: "translate";
    translateTarget: "translateTarget";
    startWorkflow: "startWorkflow";
}
```
